...
import { BarChart3, CalendarDays, ClipboardList, CreditCard, House, Inbox, MailCheck, ShieldPlus, UsersRound } from 'lucide-react'
...
export function AdminShell({ children, summary }: AdminShellProps) {
  ...
  return (
    <div className="wellstudio-admin-shell min-h-screen bg-transparent text-[var(--foreground)] xl:h-screen xl:overflow-hidden">
        ...
        <div className="grid flex-1 gap-4 xl:h-full xl:grid-cols-[264px_minmax(0,1fr)] xl:items-stretch xl:gap-0">
          <aside className="hidden xl:block xl:min-h-0 xl:p-4 xl:pr-0">
            <div className="sticky top-4 flex h-[calc(100vh-2rem)] flex-col">
              <div className="flex min-h-0 flex-1 flex-col overflow-y-auto rounded-[1.65rem] border border-white/8 bg-[linear-gradient(180deg,#111317_0%,#171a20_100%)] p-4 text-white shadow-[0_22px_62px_rgba(17,18,22,0.18)]">
                ...
                <nav aria-label="Navegación admin" className="mt-4 flex flex-col gap-0.5">
                  {adminNavItems.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        className={cn(
                          'relative flex items-center gap-3 overflow-hidden rounded-[1.2rem] px-3.5 py-2 text-sm font-medium transition-[background-color,color,box-shadow] duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50',
                          isActive
      ...
      >
        <div ref={mobileNavScrollerRef} className="mx-auto flex w-full max-w-[calc(100vw-1rem)] items-center justify-start gap-1 overflow-x-auto rounded-[2rem] border border-[color:color-mix(in_srgb,var(--wellstudio-blue)_8%,white)] bg-white/58 p-1.5 shadow-[0_10px_28px_rgba(18,20,24,0.07)] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:gap-2">
          {adminNavItems.map((item) => {
            const Icon = item.icon
            const isActive = isAdminNavItemActive(pathname, item.href)
            const isPendingTarget =
